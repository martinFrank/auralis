...
class OocResponseAgentTest {
    ...
    @Test
    void respondsToPauseRequest() throws IOException {
        Context context = loadContext("Wartet kurz, ich muss kurz aufs Klo.",
                "Pause-Anfrage, freundlich bestaetigen.");
        runAndPrint(context);
    }

    @Test
    void respondsToRecapRequest() throws IOException {
        Context context = loadContext("Sag mal, was war nochmal mein aktuelles Ziel?",
                "Recap-Anfrage, kurzen Status geben (Quest-Titel, Ort, Tageszeit).");
        runAndPrint(context);
    }

    private static Context loadContext(String playerInput, String hints) throws IOException {
        InputStream in = OocResponseAgentTest.class.getResourceAsStream(RESOURCE);
        ...
        return new Context(quest, location, "spaet abends", hints, playerInput);
    }

    private static void runAndPrint(Context context) {
        ChatLanguageModel model = chatModel();
        OocResponseAgent agent = new OocResponseAgent(model);
